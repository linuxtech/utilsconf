...
use warnings;
use utf8;
my ($PW, $SH, $GR, %PASSWD, %SHADOW, %GROUP, %UID, %UID_NAME, %GID, %GID_NAME);
...
foreach my $user (keys %PASSWD){
	if(!defined($GID{$PASSWD{$user}->{'gid'}})){print "User $user has non-existent group number: $PASSWD{$user}->{'gid'}\n"}
}
...
foreach my $user (keys %PASSWD){
	if($PASSWD{$user}->{'uid'}>=500 && -d $PASSWD{$user}->{'home'}){
		...
		if($user ne $owner){
			print "Homedir of $user is owned by user '$owner'\n";
		}
	}
}
...
#Ensure users don't have key-based access
foreach my $user (keys %PASSWD){
	if( -s "$PASSWD{$user}->{'home'}/.ssh/authorized_keys"){print "User $user has non-empty authorized_keys file\n"}
}

#List of users with shell access
foreach my $user (keys %PASSWD){
	if(	$PASSWD{$user}->{'shell'} ne '/usr/sbin/nologin' && 
